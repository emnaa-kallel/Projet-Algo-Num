...
# (couleur, style de ligne, marqueur) — lisible en couleur ET en N&B
_METHOD_STYLES = [
    ("#E63946", "-",  "o"),
    ("#2196F3", "--", "s"),
    ("#4CAF50", "-",  "^"),
...
class Visualizer:
    ...
    # ─── 2. Convergence (méthode centrale enrichie) ───────────────────────────
    ...
    # ─── 3. Comparaison méthodes d'intégration (helper → plot_convergence) ────
    ...
    def plot_runge_phenomenon(self, x_fine, y_true, interpolations):
        with plt.style.context(self.style):
            fig, ax = plt.subplots(figsize=self.figsize)
            ax.plot(x_fine, y_true, color="#c9d1d9", lw=2,
                    linestyle="--", label="Fonction exacte")
            for idx, (name, y_interp) in enumerate(interpolations.items()):
                if name in ("x_fine", "y_true"):
                    continue
                sty = _get_method_style(idx)
                ax.plot(x_fine, y_interp, lw=2, label=name,
    ...
    def plot_cooling_analysis(self, t_data, T_data, t_fine, T_interp, k_opt, T_model):
        with plt.style.context(self.style):
            fig, ax = plt.subplots(figsize=self.figsize)
            ax.scatter(t_data, T_data, color=_COLORS["data"], zorder=6, s=70,
                       label="Données mesurées")
            ax.plot(t_fine, T_interp, color=_COLORS["newton"], lw=2,
                    label="Interpolation Newton")
            ax.plot(t_fine, T_model, color=_COLORS["model"], lw=2, linestyle="--",
    ...
    def plot_flow_analysis(self, x_data, v_data, x_fine, v_interp, w_function):
        ...
        with plt.style.context(self.style):
            ...
            ax1.scatter(x_data, v_data, color=_COLORS["data"], zorder=5, s=60,
                        label="Données v(x)")
            ax1.plot(x_fine, v_interp, color=_COLORS["vx"], lw=2, label="v(x) interpolé")
            ax1.set_xlabel("x (m)"); ax1.set_ylabel("v (m/s)"); ax1.set_title("Vitesse v(x)")
            ax1.legend(fontsize=10); ax1.grid(True, linestyle="--", alpha=0.4)
            ax2.plot(x_fine, vw_fine, color=_COLORS["vwx"], lw=2, label="v(x)·w(x)")
            ax2.fill_between(x_fine, vw_fine, alpha=0.25, color=_COLORS["vwx"])
            ax2.set_xlabel("x (m)"); ax2.set_ylabel("v·w (m²/s)")
